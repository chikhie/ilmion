<template>
  <div class="w-full mb-4">
    ...
    <div class="w-full">
        ...
        <!-- Main Panel -->
        <div class="w-full bg-brand-dark/95 backdrop-blur-md border border-brand-gold/30 p-4 rounded-2xl shadow-xl transition-all duration-300">
            
            <!-- Not Connected State -->
            <div v-if="!isConnected" class="flex flex-col md:flex-row gap-3 justify-center items-center">
                 <button @click="startJoin" class="w-full md:w-auto px-6 bg-brand-wood text-brand-parchment py-2 rounded-xl font-bold font-serif-title shadow-lg hover:bg-brand-dark hover:scale-[1.02] transition-all border border-brand-gold/20 flex items-center justify-center gap-2 text-sm">
                 ...
                 <button @click="createGroup" class="w-full md:w-auto px-6 bg-brand-gold text-brand-dark py-2 rounded-xl font-bold font-serif-title shadow-lg hover:bg-white hover:scale-[1.02] transition-all flex items-center justify-center gap-2 text-sm">
            ...
            <!-- Active Lobby State -->
            <div v-else>
                 <div class="flex justify-between items-center mb-2 border-b border-brand-gold/10 pb-2">
                     <h3 class="font-serif-title font-bold text-lg text-brand-gold flex items-center gap-2">
                 ...
                 <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <!-- CODE DISPLAY -->
                     <div class="bg-black/40 rounded-xl p-3 text-center cursor-pointer group hover:bg-black/50 transition-colors border-2 h-full flex flex-col justify-center" :class="copied ? 'border-green-500/50' : 'border-dashed border-brand-wood/30'" @click="copyCode">
                         <p class="text-[9px] uppercase tracking-widest text-brand-wood mb-0.5 font-bold">Code d'invitation</p>
                         <p class="text-xl font-mono font-bold text-white tracking-wider group-active:scale-95 transition-transform">{{ currentCode }}</p>
                         <p class="text-[9px] mt-1 font-bold transition-colors" :class="copied ? 'text-green-400' : 'text-brand-wood/50 group-hover:text-brand-gold'">
