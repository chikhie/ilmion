<template>
  <div class="min-h-screen bg-[#1e1f22] pt-20 pb-12 font-sans text-gray-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      
      <!-- Header Section -->
      <div class="text-center mb-16 relative">
          <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 bg-[#00B578]/5 rounded-full blur-3xl -z-10"></div>
          <h1 class="text-4xl md:text-5xl font-sans font-bold text-[#00B578] mb-6 tracking-wide drop-shadow-lg">
            Bibliothèque Vidéo
          </h1>
          <p class="text-lg md:text-xl text-gray-300/80 max-w-2xl mx-auto leading-relaxed">
      ...
      <div v-else-if="videos.length === 0" class="text-center py-20 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm">
        <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-[#00B578]/10 text-[#00B578] mb-4">
          <i class="fas fa-film text-2xl"></i>
        </div>
        <h3 class="text-xl font-bold text-white mb-2">Aucune vidéo disponible</h3>
        <p class="text-gray-300/60">Revenez bientôt pour découvrir du nouveau contenu.</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        <NuxtLink 
          ...
          :key="video" 
          :to="`/videos/${video}`"
          class="group relative bg-[#082540] rounded-2xl overflow-hidden border border-[#00B578]/20 hover:border-[#00B578]/50 transition-all duration-300 hover:shadow-[0_0_30px_rgba(195,151,18,0.15)] hover:-translate-y-1 block"
        >
          <!-- Thumbnail Container -->
          <div class="aspect-video bg-black/50 relative overflow-hidden">
             ...
             <div class="absolute inset-0 bg-gradient-to-br from-brand-gold/20 to-brand-dark/80 flex items-center justify-center">
                 <i class="fas fa-play-circle text-5xl text-white/80 group-hover:text-[#00B578] group-hover:scale-110 transition-all duration-300 drop-shadow-lg"></i>
             </div>
          ...
          <!-- Content -->
          <div class="p-6">
            <h3 class="text-xl font-bold text-gray-300 group-hover:text-[#00B578] transition-colors mb-2 line-clamp-1">
              {{ formatVideoTitle(video) }}
            </h3>
            <div class="flex items-center text-sm text-gray-300/60 gap-4">
               <span class="flex items-center gap-1">
                 <i class="fas fa-clock text-xs"></i>
                 <span>--:--</span>
               </span>
               <span class="px-2 py-0.5 rounded-full bg-[#00B578]/10 text-[#00B578] text-xs font-bold uppercase tracking-wider border border-[#00B578]/20">
                 Vidéo
               </span>
            </div>
          </div>
