<template>
  <div class="flex-grow flex flex-col items-center justify-start relative overflow-hidden py-6">
    ...
    <div class="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-8 md:pt-6 md:pb-12">
      ...
      <div v-if="filteredGames && filteredGames.length > 0" class="hidden md:grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 lg:gap-6">
        <div v-for="game in filteredGames" :key="game.id" 
          ...
          <div class="h-32 relative overflow-hidden border-b border-brand-gold/10">
            ...
            <div v-else class="w-full h-full flex items-center justify-center bg-brand-wood text-brand-gold">
               <svg v-if="game.type === 3" xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
               </svg>
              </div>
            
            <div v-if="game.isPremium" class="absolute top-2 left-2 bg-brand-gold text-brand-dark text-[10px] font-bold px-1.5 py-0.5 rounded shadow-lg uppercase tracking-wider">
              PREMIUM
            </div>
          </div>
          ...
          <div class="p-4 flex-1 flex flex-col relative z-10">
            ...
            <div class="mt-auto pt-3">
              <button @click="handlePlay(game.id)" 
                class="w-full text-center bg-brand-wood text-brand-parchment rounded-lg py-1.5 px-2 text-sm font-bold font-serif-title hover:bg-brand-dark transition-all duration-300 shadow-md border border-brand-gold/20 cursor-pointer active:scale-95">
                Jouer
              </button>
            </div>
          </div>
